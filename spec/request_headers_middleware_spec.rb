...
describe RequestHeadersMiddleware::Middleware do
  ...
  context "when MockRackApp called with a request" do
    let(:env) { Rack::MockRequest.env_for('/some/path', {'HTTP_X_REQUEST_ID' => '1234', 
                                                         'HTTP_X_FORWARD' => '123', 
    ...
    context "with blacklist" do
      ...
      it "only saves the X-Forward in RequestHeadersMiddleware.store" do
        subject.call(env)
        expect(app['CONTENT_TYPE']).to eq('text/plain')
        expect(RequestHeadersMiddleware.store).to match('X-Request-Id1': '12', 'X-Request-Id': '1234')
      end
    end
    ...
    context "with callbacks" do
      before do
        ...
        RequestHeadersMiddleware.blacklist = []
        RequestHeadersMiddleware.callbacks = [ Proc.new { |env| env['HTTP_X_REQUEST_ID'] = '4321' }, 
                                               Proc.new { |env| env['HTTP_X_REQUEST_ID1'] = '21' }]
      end
      after do
        RequestHeadersMiddleware.whitelist = []
        RequestHeadersMiddleware.blacklist = []
